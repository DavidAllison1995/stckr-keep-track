...
import { searchIcons, getIconByEmoji, getDefaultIcon, TwemojiIconData } from '@/data/twemojiIcons';
import { Search, ChevronDown } from 'lucide-react';

interface TwemojiIconPickerProps {
  selectedEmoji: string;
  onChange: (emoji: string) => void;
  className?: string;
}

const TwemojiIconPicker = ({ selectedEmoji, onChange, className = '' }: TwemojiIconPickerProps) => {
  const [searchQuery, setSearchQuery] = useState('');
  const [isOpen, setIsOpen] = useState(false);
  const [filteredIcons, setFilteredIcons] = useState<TwemojiIconData[]>([]);
  ...
  const handleIconSelect = (icon: TwemojiIconData) => {
    onChange(icon.emoji);
    setIsOpen(false);
    setSearchQuery('');
  };
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <div className="flex items-center gap-3 mb-2 p-2 bg-gray-800 border border-gray-700 rounded-lg">
        <TwemojiIcon emoji={selectedIcon.emoji} className="w-6 h-6" size={24} />
        <div>
                  ...
                  <>
                    ...
                    <div className="space-y-1">
                      {filteredIcons.map((icon, index) => (
                        ...
                        >
                          <TwemojiIcon emoji={icon.emoji} className="w-5 h-5 flex-shrink-0" size={20} />
...
export default TwemojiIconPicker;
