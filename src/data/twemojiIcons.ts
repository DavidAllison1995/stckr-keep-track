...
export interface TwemojiIconData {
  emoji: string;
  name: string;
  category: string;
  keywords: string[];
...
export const twemojiIcons: TwemojiIconData[] = [
  // Tools & Equipment
...
export const getIconByEmoji = (emoji: string): TwemojiIconData | undefined => {
  return twemojiIcons.find(icon => icon.emoji === emoji);
};

export const searchIcons = (query: string): TwemojiIconData[] => {
  if (!query.trim()) return twemojiIcons.slice(0, 20); // Show first 20 by default
  
  const lowercaseQuery = query.toLowerCase();
  return twemojiIcons.filter(icon => 
    icon.name.toLowerCase().includes(lowercaseQuery) ||
    icon.keywords.some(keyword => keyword.toLowerCase().includes(lowercaseQuery))
  );
};

export const getDefaultIcon = (): TwemojiIconData => {
  return { emoji: '📦', name: 'Package', category: 'Storage', keywords: ['box', 'package', 'storage', 'default'] };
};

export const getCategorizedIcons = () => {
  const categories: { [key: string]: TwemojiIconData[] } = {};
  
  twemojiIcons.forEach(icon => {
